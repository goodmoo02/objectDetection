#include "/home/jiwoong/objectDetection/blackbox/declare.h"
...
int rec(struct cap_dir* cd)
{
	VideoWriter writer;
	// 동영상 파일을 저장하기 위한 준비
	Size size1 = Size((int)cd->cap.get(CAP_PROP_FRAME_WIDTH),(int)cd->cap.get(CAP_PROP_FRAME_HEIGHT));
    double fps = 30.0;
	writer.open(cd->dir, VideoWriter::fourcc('M', 'J', 'P', 'G'), fps, size1, true);
	if (!writer.isOpened())
	{
